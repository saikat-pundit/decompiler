...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    */
    ...
    public final Object mo1576trySendJP2dKIU(E element) {
        Object objOfferInternal = offerInternal(element);
        if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1596successJP2dKIU(Unit.INSTANCE);
        }
        if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1595failurePtdJZtk() : ChannelResult.INSTANCE.m1594closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (objOfferInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1594closedJP2dKIU(helpCloseAndGetSendException((Closed) objOfferInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (undeliveredElementExceptionCallUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m71constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
        Result.Companion companion2 = Result.INSTANCE;
        continuation.resumeWith(Result.m71constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1621remove()) {
                receive.helpRemove();
            } else {
                objM1613constructorimpl$default = InlineList.m1618plusFjFbRPM(objM1613constructorimpl$default, receive);
            }
        }
        if (objM1613constructorimpl$default != null) {
            if (!(objM1613constructorimpl$default instanceof ArrayList)) {
                ((Receive) objM1613constructorimpl$default).resumeReceiveClosed(closed);
            } else if (objM1613constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) objM1613constructorimpl$default;
