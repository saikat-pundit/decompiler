...
$( document ).ready(function(){
	if(localStorage.getItem("image")!= null){
		...
		document.getElementById('smallImage5').src=localStorage.getItem("image5");
		document.getElementById('smallImage6').src=localStorage.getItem("image6");
	}
...
function validate_index(button_index)
{
	...
	else if($("#smallImage5").attr('src')=='images/img-load.jpg'){
		alert('Please capture photo Kitchen!');
		$("#collapse3").collapse('show');
		$("#smallImage5").focus();
		return false;
	}
	else if($("#smallImage6").attr('src')=='images/img-load.jpg'){
	...
	else
	{
		...
		base64imageconversion(buttonIndex);
		//alert("ok");
...
function after_validation(buttonIndex)
{
	...
	localStorage.setItem("image1",image[1]);
	localStorage.setItem("image2",image[2]);
	localStorage.setItem("image3",image[3]);
	localStorage.setItem("image4",image[4]);
	localStorage.setItem("image5",image[5]);
	localStorage.setItem("image6",image[6]);
	

	$('#serving_eating_picture').val(image[1]);
	$('#school_premises_picture').val(image[2]);
	$('#toilets_picture').val(image[3]);
	$('#drinking_water_source_picture').val(image[4]);
	$('#kitchen_picture').val(image[5]);
	$('#other_picture').val(image[6]);
...
/////////////////////     checking network connection
	...
	if(navigator.onLine==true) 
	{
				...
				$.ajax({
					...
					success: function(result)
					{
						...
						if(result.status=="1")
						{
							...
							localStorage.removeItem('image5');
		...
		{
			localdb(mobile_no,inspection_date,school_code_fk,totaldata,image[1],image[2],image[3],image[4],image[5],image[6]);
		}
		...
		function onConfirm(buttonIndex){
			//alert(buttonIndex);
			if(buttonIndex===1){
				localdb(mobile_no,inspection_date,school_code_fk,totaldata,image1,image2,image3,image4,image5,image6);
			}
			else if(buttonIndex===2){
				$("#preloader").fadeIn();
				$.ajax({
...
//////////////////////////////////////////////////////////
 /////////////////////////////////////////////
///ofline insert////
function localdb(mobile_no,inspection_date,school_code_fk,totaldata,image1,image2,image3,image4,image5,image6)
	{
	$("#preloader").fadeIn();
 //////////////// Securing localdata ///////////////////////
	...
	var encryptedI1 = CryptoJS.AES.encrypt(ImgText1, secret);
	var encryptedI2 = CryptoJS.AES.encrypt(ImgText2, secret);
	var encryptedI3 = CryptoJS.AES.encrypt(ImgText3, secret);
	var encryptedI4 = CryptoJS.AES.encrypt(ImgText4, secret);
	var encryptedI5 = CryptoJS.AES.encrypt(ImgText5, secret);
	var encryptedI6 = CryptoJS.AES.encrypt(ImgText6, secret);
...
async function base64imageconversion(button_index){
	for(i=1;i<=6;i++){
		
		try{
			var image_path=(document.getElementById('smallImage'+i).src);
